...
// The constructor
MummyBullet::MummyBullet()
{
	m_Sprite = Sprite(TextureHolder::GetTexture(
		"graphics/toilet-paper.png"));
	m_Sprite.setScale(.30, .30);

	m_rotateSpeed = 180.0f;
}

void MummyBullet::shoot(float startX, float startY,
...
void MummyBullet::rotate(float elapsedTime)
{
	float rotationAngle = m_rotateSpeed * elapsedTime;
	m_Sprite.rotate(rotationAngle);
}
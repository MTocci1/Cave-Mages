...
void Water::spawn(IntRect arena, Vector2f resolution, int tileSize)
{
	...
	if (m_Position.x > m_Arena.width - 500) {
		m_Position.x = m_Arena.width - 500;
	}

	if (m_Position.y > m_Arena.height - 500) {
		m_Position.y = m_Arena.height - 500;
	}
